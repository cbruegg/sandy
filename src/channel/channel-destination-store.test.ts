...
import { ChannelDestinationStore } from "./channel-destination-store.js";
...
test("ChannelDestinationStore persists the default chat id", async () => {
  ...
  try {
    const store = new ChannelDestinationStore(configDirectory);
    assert.equal(await store.getDefaultChatId(), null);
    await store.setDefaultChatId("chat-1");
    assert.equal(await new ChannelDestinationStore(configDirectory).getDefaultChatId(), "chat-1");
  } finally {
