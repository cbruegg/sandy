...
test("archiveSkill moves the skill directory into the archive root with a UUID", async () => {
  const configDirectory = await createTempConfigDirectory();
  try {
    const service = new SkillService(configDirectory);
    await service.createSkill({
    ...
    const archivePath = await service.archiveSkill("todoist");

    // Skill should be gone from the live skills directory
    assert.equal(existsSync(join(configDirectory, "skills", "todoist")), false);

    // Archive path should exist
    assert.equal(existsSync(archivePath), true);

    // Archive path should be under archive/skills
    assert.ok(archivePath.includes("archive/skills/todoist-"));

    // The skill should no longer be discovered by getSkills
    assert.equal(service.getSkills().length, 0);
  } finally {
    await rm(configDirectory, { recursive: true, force: true });
  }
});

test("archiveSkill rejects skills that do not exist", async () => {
  const configDirectory = await createTempConfigDirectory();
  try {
    const service = new SkillService(configDirectory);

    await assert.rejects(
      service.archiveSkill("missing"),
      /does not exist/,
    );
  } finally {
    await rm(configDirectory, { recursive: true, force: true });
...
test("archiveSkill rejects invalid skillIds", async () => {
  const configDirectory = await createTempConfigDirectory();
  try {
    const service = new SkillService(configDirectory);
