...
import { channelStateFile, jobApprovalsFile, jobWorkspace, jobWorkspaceRoot, jobsFile, jobsRoot, matrixStateRoot, sandyStateRoot } from "./state-paths.js";
...
test("state path helpers centralize persisted locations", () => {
  ...
  assert.equal(jobApprovalsFile(configDirectory), join(configDirectory, "state", "jobs", "job-approvals.json"));
